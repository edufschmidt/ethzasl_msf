 ...
 */
#ifndef POSE_VELOCITY_ALTITUDE_MEASUREMENTMANAGER_H
#define POSE_VELOCITY_ALTITUDE_MEASUREMENTMANAGER_H
...
#include <msf_updates/PosePositionVelocityAltitudeSensorConfig.h>
#include <msf_updates/SinglePoseSensorConfig.h>

namespace msf_position_pose_velocity_altitude_sensor {

typedef msf_updates::PosePositionVelocityAltitudeSensorConfig Config_T;
typedef dynamic_reconfigure::Server<Config_T> ReconfigureServer;
typedef shared_ptr<ReconfigureServer> ReconfigureServerPtr;

class PosePositionVelocityAltitudeSensorManager : public msf_core::MSF_SensorManagerROS<
        msf_updates::EKFState> {
    typedef msf_pose_sensor::PoseSensorHandler<
    msf_updates::pose_measurement::PoseMeasurement<>, PosePositionVelocityAltitudeSensorManager> PoseSensorHandler_T;
    friend class msf_pose_sensor::PoseSensorHandler<
            msf_updates::pose_measurement::PoseMeasurement<>, PosePositionVelocityAltitudeSensorManager>;
    typedef msf_position_sensor::PositionSensorHandler<
    msf_updates::position_measurement::PositionMeasurement, PosePositionVelocityAltitudeSensorManager> PositionSensorHandler_T;
    friend class msf_position_sensor::PositionSensorHandler<
            msf_updates::position_measurement::PositionMeasurement, PosePositionVelocityAltitudeSensorManager>;
public:
    typedef msf_updates::EKFState EKFState_T;
    typedef EKFState_T::StateSequence_T StateSequence_T;
    typedef EKFState_T::StateDefinition_T StateDefinition_T;

    PosePositionVelocityAltitudeSensorManager(
        ...
        position_handler_.reset(
                    new PositionSensorHandler_T(*this, "",
                                                "position_sensor"));
        ...
        ReconfigureServer::CallbackType f = boost::bind(
                    &PosePositionVelocityAltitudeSensorManager::Config, this, _1, _2);
        reconf_server_->setCallback(f);
    }

    virtual ~PosePositionVelocityAltitudeSensorManager() {
    }
...
private:
   ...
   */
    virtual void Config(Config_T &config, uint32_t level) {
        ...
        position_handler_->SetNoises(config.noise_meas_rtk);
        velocity_handler_->SetNoises(config.noise_meas_v);
        altitude_handler_->SetNoises(config.altitude_meas);
    }

    void Init(double scale) const {
        Eigen::Matrix<double, 3, 1> p, v, b_w, b_a, g, w_m, a_m, p_ic, p_vc, p_ip;
...
#endif  // POSE_VELOCITY_ALTITUDE_MEASUREMENTMANAGER_H
