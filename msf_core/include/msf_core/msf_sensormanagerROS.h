...
private:
    ...
    ros::Publisher pubOdomAfterUpdate_;  ///< Publishes 6DoF pose output after the update has been applied.
...
public:
    ...
    MSF_SensorManagerROS(ros::NodeHandle pnh = ros::NodeHandle("~core")) {
        ...
        pubOdomAfterUpdate_ = nh.advertise
                < nav_msgs::Odometry > ("odom_after_update", 100);
   ...
   */
    ...
    virtual void PublishStateAfterUpdate(
        ...
        if (pubOdomAfterUpdate_.getNumSubscribers()) {
            // Publish odom after correction with covariance.
            nav_msgs::Odometry msgOdom;
            msgOdom.header.stamp = ros::Time(state->time);
            msgOdom.header.seq = msg_seq;
            msgOdom.header.frame_id = "odom";
            msgOdom.child_frame_id  = "base_link";

            state->ToOdometryMsg(msgOdom);
            pubOdomAfterUpdate_.publish(msgOdom);
        }
